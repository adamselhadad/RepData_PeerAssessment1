...
```
There are `r naRows` rows with an NA in them.  
...
Now let's comare these new nubmers to the originals.  The mean is now `r as.character(round(mean(stepsPerDayMod$Steps), digits = 2))` compared to `r as.character(round(mean(stepsPerDay$Steps), digits = 2))`.  
The median is now `r as.character(round(median(stepsPerDayMod$Steps), digits =2))` compared to `r as.character(median(stepsPerDay$Steps))`.  We can see that the mean didn't change.  The median however did change.  It also became a non-interger value because we replaced some numbers with means.
